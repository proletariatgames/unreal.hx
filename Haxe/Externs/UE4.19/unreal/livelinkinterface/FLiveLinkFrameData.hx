/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.livelinkinterface;

@:umodule("LiveLinkInterface")
@:glueCppIncludes("Public/LiveLinkTypes.h")
@:noCopy @:noEquals @:uextern @:ustruct extern class FLiveLinkFrameData {
  @:uproperty public var MetaData : unreal.livelinkinterface.FLiveLinkMetaData;
  @:uproperty public var WorldTime : unreal.livelinkinterface.FLiveLinkWorldTime;
  @:uproperty public var CurveElements : unreal.TArray<unreal.livelinkinterface.FLiveLinkCurveElement>;
  @:uproperty public var Transforms : unreal.TArray<unreal.FTransform>;
  
}
