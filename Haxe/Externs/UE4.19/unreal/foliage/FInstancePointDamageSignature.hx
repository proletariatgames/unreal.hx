/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.foliage;

/**
  @param InstanceIndex
  @param Damage
  @param InstigatedBy
  @param HitLocation
  @param ShotFromDirection
  @param DamageType
  @param DamageCauser
  
**/
@:glueCppIncludes("Public/FoliageInstancedStaticMeshComponent.h")
@:uParamName("InstanceIndex")
@:uParamName("Damage")
@:uParamName("InstigatedBy")
@:uParamName("HitLocation")
@:uParamName("ShotFromDirection")
@:uParamName("DamageType")
@:uParamName("DamageCauser")
@:umodule("Foliage")
typedef FInstancePointDamageSignature = unreal.DynamicMulticastDelegate<FInstancePointDamageSignature, unreal.Int32->unreal.Float32->unreal.AController->unreal.FVector->unreal.FVector->unreal.Const<unreal.UDamageType>->unreal.AActor->Void>;