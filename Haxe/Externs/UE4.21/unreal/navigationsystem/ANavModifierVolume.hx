/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.navigationsystem;

/**
  Allows applying selected AreaClass to navmesh, using Volume's shape
**/
@:umodule("NavigationSystem")
@:glueCppIncludes("NavModifierVolume.h")
@:uextern @:uclass extern class ANavModifierVolume extends unreal.AVolume implements unreal.INavRelevantInterface {
  @:uproperty private var AreaClass : unreal.TSubclassOf<unreal.navigationsystem.UNavArea>;
  @:ufunction(BlueprintCallable) @:final public function SetAreaClass(@:opt("None") NewAreaClass : unreal.TSubclassOf<unreal.navigationsystem.UNavArea>) : Void;
  // NavRelevantInterface interface implementation
  
}
