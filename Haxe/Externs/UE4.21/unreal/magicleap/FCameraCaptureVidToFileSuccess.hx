/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.magicleap;

/**
  Delegate used to notify the instigating blueprint of a capture video to file success.
  
     @param FilePath A string containing the path to the newly created mp4.
  @param FilePath
  
**/
@:glueCppIncludes("Classes/CameraCaptureComponent.h")
@:uParamName("FilePath")
@:umodule("MagicLeap")
@:uname("UCameraCaptureComponent.FCameraCaptureVidToFileSuccess")
typedef FCameraCaptureVidToFileSuccess = unreal.DynamicMulticastDelegate<FCameraCaptureVidToFileSuccess, unreal.FString->Void>;