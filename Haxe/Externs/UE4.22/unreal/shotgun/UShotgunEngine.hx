/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.shotgun;

/**
  WARNING: This type was not defined as DLL export on its declaration. Because of that, some of its methods are inaccessible
  
  Wrapper for the Python Shotgun Engine
  The functions are implemented in Python by a class that derives from this one
**/
@:umodule("Shotgun")
@:glueCppIncludes("Private/ShotgunEngine.h")
@:noClass @:uextern @:uclass extern class UShotgunEngine extends unreal.UObject {
  
  /**
    Selected actors to be used for Shotgun commands
  **/
  @:uproperty public var SelectedActors : unreal.TArray<unreal.AActor>;
  
  /**
    Selected assets to be used for Shotgun commands
  **/
  @:uproperty public var SelectedAssets : unreal.TArray<unreal.assetregistry.FAssetData>;
  
}
