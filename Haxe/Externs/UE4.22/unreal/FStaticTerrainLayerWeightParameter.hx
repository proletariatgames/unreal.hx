/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal;

/**
  Holds the information for a static switch parameter
**/
@:glueCppIncludes("Public/StaticParameterSet.h")
@:noCopy @:noEquals @:uextern @:ustruct extern class FStaticTerrainLayerWeightParameter {
  @:uproperty public var bWeightBasedBlend : Bool;
  @:uproperty public var WeightmapIndex : unreal.Int32;
  @:uproperty public var ExpressionGUID : unreal.FGuid;
  @:uproperty public var bOverride : Bool;
  @:uproperty public var ParameterInfo : unreal.FMaterialParameterInfo;
  
}
