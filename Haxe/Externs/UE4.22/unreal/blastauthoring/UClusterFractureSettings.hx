/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.blastauthoring;

@:umodule("BlastAuthoring")
@:glueCppIncludes("MeshFractureSettings.h")
@:uextern @:uclass extern class UClusterFractureSettings extends unreal.UObject {
  
  /**
    Clusters Radius - Clustered Voronoi Method
  **/
  @:uproperty public var ClusterRadius : unreal.Float32;
  
  /**
    Sites per of Clusters - Clustered Voronoi Method
  **/
  @:uproperty public var SitesPerCluster : unreal.Float32;
  
  /**
    Number of Clusters - Clustered Voronoi Method
  **/
  @:uproperty public var NumberClusters : unreal.Float32;
  
}
