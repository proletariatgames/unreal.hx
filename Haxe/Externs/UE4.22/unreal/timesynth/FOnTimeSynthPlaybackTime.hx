/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.timesynth;

/**
  Called to get playback time progress callbacks. Time is based off the synth time clock, not game thread time so time will be accurate relative to the synth (minus thread communication latency).
  @param SynthPlaybackTimeSeconds
  
**/
@:glueCppIncludes("Classes/TimeSynthComponent.h")
@:uParamName("SynthPlaybackTimeSeconds")
@:umodule("TimeSynth")
typedef FOnTimeSynthPlaybackTime = unreal.DynamicMulticastDelegate<FOnTimeSynthPlaybackTime, unreal.Float32->Void>;