/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal;

@:glueCppIncludes("Animation/AnimCurveCompressionCodec_CompressedRichCurve.h")
@:uextern @:uclass extern class UAnimCurveCompressionCodec_CompressedRichCurve extends unreal.UAnimCurveCompressionCodec {
  #if WITH_EDITORONLY_DATA
  
  /**
    Sample rate to use when measuring the curve error
  **/
  @:uproperty public var ErrorSampleRate : unreal.Float32;
  
  /**
    Whether to use the animation sequence sample rate or an explicit value
  **/
  @:uproperty public var UseAnimSequenceSampleRate : Bool;
  
  /**
    Max error allowed when compressing the rich curves
  **/
  @:uproperty public var MaxCurveError : unreal.Float32;
  #end // WITH_EDITORONLY_DATA
  
}
