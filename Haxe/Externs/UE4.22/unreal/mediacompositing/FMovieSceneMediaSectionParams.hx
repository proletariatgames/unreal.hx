/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.mediacompositing;

@:umodule("MediaCompositing")
@:glueCppIncludes("Private/MovieScene/MovieSceneMediaTemplate.h")
@:noCopy @:noEquals @:uextern @:ustruct extern class FMovieSceneMediaSectionParams {
  @:uproperty public var StartFrameOffset : unreal.FFrameNumber;
  @:uproperty public var bLooping : Bool;
  @:uproperty public var SectionEndFrame : unreal.FFrameNumber;
  @:uproperty public var SectionStartFrame : unreal.FFrameNumber;
  @:uproperty public var MediaPlayer : unreal.mediaassets.UMediaPlayer;
  @:uproperty public var MediaTexture : unreal.mediaassets.UMediaTexture;
  @:uproperty public var MediaSource : unreal.mediaassets.UMediaSource;
  @:uproperty public var MediaSoundComponent : unreal.mediaassets.UMediaSoundComponent;
  
}
