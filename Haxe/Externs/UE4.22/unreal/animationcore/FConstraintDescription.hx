/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.animationcore;

/**
  A description of how to apply a simple transform constraint
**/
@:umodule("AnimationCore")
@:glueCppIncludes("Public/Constraint.h")
@:uextern @:ustruct extern class FConstraintDescription {
  @:uproperty public var ScaleAxes : unreal.animationcore.FFilterOptionPerAxis;
  @:uproperty public var RotationAxes : unreal.animationcore.FFilterOptionPerAxis;
  @:uproperty public var TranslationAxes : unreal.animationcore.FFilterOptionPerAxis;
  
  /**
    this does composed transform - where as individual will accumulate per component
  **/
  @:uproperty public var bParent : Bool;
  @:uproperty public var bScale : Bool;
  @:uproperty public var bRotation : Bool;
  @:uproperty public var bTranslation : Bool;
  
}
