...
class Package
{
  public static function main(gameSource:String, mappedFiles:Array<{ name:String, target:String }>, buildFiles:{ files:Array<String>, targetExternal:String })
  {
    ...
    for (file in buildFiles.files) {
      ...
      var className = name.charAt(0).toUpperCase() + name.substr(1) + (isBuild ? 'Build' : 'Target');
    ...
    for (name in names) {
      var tl = Context.getModule(name.className); // make sure it's compiled
      var isMaster = false,
          curType = null;
      for (t in tl) {
        if (curType == null) {
        ...
        if (Context.unify(t, masterT) && !Context.unify(t, program)) {
          isMaster = true;
          curType = t;
          break;
        }
      ...
      if (isMaster) {
        if (masterModule == null) {
          masterModule = curType;
        } else {
          Context.error('More than one HaxeModuleRules module found: $curType and $masterModule', Context.currentPos());
        }
      }
      switch (curType) {
      case TInst(cl,_):
...
#if (UE_VER <= 4.16)
      dst.writeString('#define UE_OLDER_416\n');
#end
