...
class ExternBaker {
  ...
  public static function process(classpaths:Array<String>, force:Bool) {
      ...
      function traverse() {
        ...
        for (file in files) {
          if (file.endsWith('.hx')) {
            ...
            filesToCompile[fileName] = { module:module, file:fileName };
          }
        }
        for (file in files) {
    ...
    function generatedSourceIsValid(srcFile:String, generatedFile:String, info:GeneratedInfo):Bool {
      ...
      if (info.srcFile != srcFile && FileSystem.exists(info.srcFile) && !filesToCompile.exists(info.srcFile)) {
    ...
    for (ref in filesToCompile) {
      var generatedHeader = getGeneratedHeader({ srcFile: ref.file, ver:1 });
